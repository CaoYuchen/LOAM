...
double laser_wrt_imu_yaw = 0;
double laser_wrt_imu_pitch = 0;
double laser_wrt_imu_roll = 0;

double voxelSize = 0.2;
float voxelSizeScale = 1.0;
double decayDistance = 50.0;

double throttleTimeLB = 0.1;
double throttleTimeUB = 0.15;
double autoVoxelMeanTime = 0.05;
bool useAutoVoxel = true;

double maxElevDiff = 0.1;
int minStableFrameNum = 2;
int minStableFrameCount = minStableFrameNum;
...
double systemTimeLaserMatching = 0;
...
int laserVoxelCornerInd[laserVoxelNum];
int laserVoxelSurfInd[laserVoxelNum];
...
void transformUpdate()
{
  if (imuPointerLast >= 0) {
    ...
    transformTobeMapped[0] = imuPitchLast;
    transformTobeMapped[2] = imuRollLast;
  }
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("laser_wrt_imu_yaw", laser_wrt_imu_yaw);
  nhPrivate.getParam("laser_wrt_imu_pitch", laser_wrt_imu_pitch);
  nhPrivate.getParam("laser_wrt_imu_roll", laser_wrt_imu_roll);
  nhPrivate.getParam("decayDistance", decayDistance);
  nhPrivate.getParam("throttleTimeLB", throttleTimeLB);
  nhPrivate.getParam("throttleTimeUB", throttleTimeUB);
  nhPrivate.getParam("autoVoxelMeanTime", autoVoxelMeanTime);
  nhPrivate.getParam("useAutoVoxel", useAutoVoxel);
  nhPrivate.getParam("maxElevDiff", maxElevDiff);
  nhPrivate.getParam("minStableFrameNum", minStableFrameNum);
  ...
  odomAftMapped.header.frame_id = "sensor_init_rot";
  odomAftMapped.child_frame_id = "aft_mapped";

  tf::TransformBroadcaster tfBroadcaster;
  tf::StampedTransform aftMappedTrans;
  aftMappedTrans.frame_id_ = "sensor_init_rot";
  ...
  downSizeFilterCorner.setLeafSize(voxelSize, voxelSize, voxelSize);

  pcl::VoxelGrid<pcl::PointXYZI> downSizeFilterSurf;
  downSizeFilterSurf.setLeafSize(2.0 * voxelSize, 2.0 * voxelSize, 2.0 * voxelSize);

  pcl::VoxelGrid<pcl::PointXYZI> downSizeFilterMap;
  downSizeFilterMap.setLeafSize(3.0 * voxelSize, 3.0 * voxelSize, 3.0 * voxelSize);
  ...
  while (status) {
    ...
    if (newLaserCloudCornerLast && newLaserCloudSurfLast && newLaserCloudFullRes && newLaserOdometry &&
      ...
      systemTimeLaserMatching = ros::Time::now().toSec();
      ...
      for (int i = 0; i < laserVoxelNum; i++) {
        laserVoxelCornerInd[i] = 0;
        laserVoxelSurfInd[i] = 0;
      }

      for (int i = 0; i < laserCloudCornerStackNum; i++) {
        pointAssociateToMap(&laserCloudCornerStack->points[i], &pointSel);
        int voxelI = int((pointSel.x - transformTobeMapped[3] + 2.5) / 5.0) + laserVoxelCenWidth;
        int voxelJ = int((pointSel.y - transformTobeMapped[4] + 2.5) / 5.0) + laserVoxelCenHeight;
        int voxelK = int((pointSel.z - transformTobeMapped[5] + 2.5) / 5.0) + laserVoxelCenDepth;

        if (pointSel.x - transformTobeMapped[3] + 2.5 < 0) voxelI--;
        if (pointSel.y - transformTobeMapped[4] + 2.5 < 0) voxelJ--;
        if (pointSel.z - transformTobeMapped[5] + 2.5 < 0) voxelK--;
          ...
          for (int jj = voxelJ - 1; jj <= voxelJ + 1; jj++) {
            for (int kk = voxelK - 1; kk <= voxelK + 1; kk++) {
              if (ii >= 0 && ii < laserVoxelWidth && jj >= 0 && 
                ...
                laserVoxelCornerInd[ii + laserVoxelWidth * jj + laserVoxelWidth * laserVoxelHeight * kk] = 1;
              }
            }
          }
        }
        ...
        for (int ii = voxelI - 1; ii <= voxelI + 1; ii++) {
          for (int jj = voxelJ - 1; jj <= voxelJ + 1; jj++) {
            for (int kk = voxelK - 1; kk <= voxelK + 1; kk++) {
              if (ii >= 0 && ii < laserVoxelWidth && jj >= 0 && 
                  jj < laserVoxelHeight && kk >= 0 && kk < laserVoxelDepth) {
                laserVoxelSurfInd[ii + laserVoxelWidth * jj + laserVoxelWidth * laserVoxelHeight * kk] = 1;
              }
            }
          }
        }
      ...
      pcl::PointCloud<pcl::PointXYZI>::Ptr cellCornerPtr, cellSurfPtr;
      for (int i = 0; i < laserCloudValidNum; i++) {
        cellCornerPtr = laserCloudCornerArray[laserCloudValidInd[i]];
        int cellCornerNum = cellCornerPtr->points.size();
        for (int j = 0; j < cellCornerNum; j++) {
          if (travelDistance - cellCornerPtr->points[j].intensity > decayDistance && decayDistance > 0) {
            cellCornerPtr->erase(cellCornerPtr->begin() + j);
            cellCornerNum--;
            j--;
            continue;
          }

          int voxelI = int((cellCornerPtr->points[j].x - transformTobeMapped[3] + 2.5) / 5.0) + laserVoxelCenWidth;
          int voxelJ = int((cellCornerPtr->points[j].y - transformTobeMapped[4] + 2.5) / 5.0) + laserVoxelCenHeight;
          int voxelK = int((cellCornerPtr->points[j].z - transformTobeMapped[5] + 2.5) / 5.0) + laserVoxelCenDepth;

          if (cellCornerPtr->points[j].x - transformTobeMapped[3] + 2.5 < 0) voxelI--;
          if (cellCornerPtr->points[j].y - transformTobeMapped[4] + 2.5 < 0) voxelJ--;
          if (cellCornerPtr->points[j].z - transformTobeMapped[5] + 2.5 < 0) voxelK--;
          ...
          if (voxelI >= 0 && voxelI < laserVoxelWidth && voxelJ >= 0 && 
            ...
            if (laserVoxelCornerInd[voxelID] == 1) {
              laserCloudCornerFromMap->push_back(cellCornerPtr->points[j]);
            }
          }
        }

        cellSurfPtr = laserCloudSurfArray[laserCloudValidInd[i]];
        int cellSurfNum = cellSurfPtr->points.size();
        for (int j = 0; j < cellSurfNum; j++) {
          if (travelDistance - cellSurfPtr->points[j].intensity > decayDistance && decayDistance > 0) {
            cellSurfPtr->erase(cellSurfPtr->begin() + j);
            cellSurfNum--;
            j--;
            continue;
          }

          int voxelI = int((cellSurfPtr->points[j].x - transformTobeMapped[3] + 2.5) / 5.0) + laserVoxelCenWidth;
          int voxelJ = int((cellSurfPtr->points[j].y - transformTobeMapped[4] + 2.5) / 5.0) + laserVoxelCenHeight;
          int voxelK = int((cellSurfPtr->points[j].z - transformTobeMapped[5] + 2.5) / 5.0) + laserVoxelCenDepth;

          if (cellSurfPtr->points[j].x - transformTobeMapped[3] + 2.5 < 0) voxelI--;
          if (cellSurfPtr->points[j].y - transformTobeMapped[4] + 2.5 < 0) voxelJ--;
          if (cellSurfPtr->points[j].z - transformTobeMapped[5] + 2.5 < 0) voxelK--;

          if (voxelI >= 0 && voxelI < laserVoxelWidth && voxelJ >= 0 && 
              voxelJ < laserVoxelHeight && voxelK >= 0 && voxelK < laserVoxelDepth) {
            int voxelID = voxelI + laserVoxelWidth * voxelJ + laserVoxelWidth * laserVoxelHeight * voxelK;
            if (laserVoxelSurfInd[voxelID] == 1) {
              laserCloudSurfFromMap->push_back(cellSurfPtr->points[j]);
            }
          }
        }
      }
      ...
      if (laserCloudCornerFromMapNum > 10 && laserCloudSurfFromMapNum > 100) {
        ...
        int* searchIndCornerArray = new int[5 * laserCloudCornerStackNum];
        int* searchIndSurfArray = new int[5 * laserCloudSurfStackNum];
        ...
        for (int iterCount = 0; iterCount < 10; iterCount++) {
          ...
          for (int i = 0; i < laserCloudCornerStackNum; i++) {
            ...
            if (isPointSel) {
              ...
              for (int j = 0; j < 5; j++) {
                searchIndCornerArray[5 * i + j] = pointSearchInd[j];
              }
            } else {
              pointSearchInd.resize(5);
              pointSearchSqDis.resize(5);
              for (int j = 0; j < 5; j++) {
                int ind = searchIndCornerArray[5 * i + j];
                float sx = laserCloudCornerFromMap->points[ind].x;
                float sy = laserCloudCornerFromMap->points[ind].y;
                float sz = laserCloudCornerFromMap->points[ind].z;
          ...
          for (int i = 0; i < laserCloudSurfStackNum; i++) {
            ...
            if (isPointSel) {
              ...
              for (int j = 0; j < 5; j++) {
                searchIndSurfArray[5 * i + j] = pointSearchInd[j];
              }
            } else {
              pointSearchInd.resize(5);
              pointSearchSqDis.resize(5);
              for (int j = 0; j < 5; j++) {
                int ind = searchIndSurfArray[5 * i + j];
                float sx = laserCloudSurfFromMap->points[ind].x;
                float sy = laserCloudSurfFromMap->points[ind].y;
                float sz = laserCloudSurfFromMap->points[ind].z;
                pointSearchInd[j] = ind;
                pointSearchSqDis[j] = (pointSel.x - sx) * (pointSel.x - sx) + (pointSel.y - sy) * (pointSel.y - sy)
                                    + (pointSel.z - sz) * (pointSel.z - sz);
              }
          ...
          float deltaR = sqrt(matX.at<float>(1, 0) * 180 / PI * matX.at<float>(1, 0) * 180 / PI + 0.05 * 0.05 * 2.0 / 3.0);
          ...
          if (iterCount >= 9 && (deltaR > 0.5 || deltaT > 0.5)) minStableFrameCount = 0;

          //ROS_INFO ("iter: %d, deltaR: %f, deltaT: %f", iterCount, deltaR, deltaT);
        }

        travelY = fabs(transformTobeMapped[4] - transformAftMapped[4]);
        travelDistance += sqrt((transformTobeMapped[3] - transformAftMapped[3]) 
                       * (transformTobeMapped[3] - transformAftMapped[3]) 
        ...
        if (travelY > maxElevDiff) minStableFrameCount = 0;
        else minStableFrameCount++;
        if (minStableFrameCount >= minStableFrameNum) transformRecY = transformTobeMapped[4];

        delete[] searchIndCornerArray;
        delete[] searchIndSurfArray;
      } else {
        minStableFrameCount = minStableFrameNum;
      }

      if (minStableFrameCount >= minStableFrameNum) {
        for (int i = 0; i < laserCloudCornerStackNum; i++) {
          pointAssociateToMap(&laserCloudCornerStack->points[i], &pointSel);
          ...
          if (cubeI >= 0 && cubeI < laserCloudWidth && 
            ...
            pointSel.intensity = travelDistance;
        ...
        double processingTime = ros::Time::now().toSec() - systemTimeLaserMatching;
        if (voxelSizeScale > 1.25 && processingTime < throttleTimeLB) voxelSizeScale -= 0.5;
        if (voxelSizeScale < 1.75 && processingTime > throttleTimeUB) voxelSizeScale += 0.5;

        if (useAutoVoxel) {
          if (processingTime < autoVoxelMeanTime) voxelSize -= 0.002;
          else voxelSize += 0.002;

          if (voxelSize < 0.1) voxelSize = 0.1;
          if (voxelSize > 0.2) voxelSize = 0.2;
        }

        if (voxelSizeScale > 1.25 || useAutoVoxel) {
          downSizeFilterCorner.setLeafSize(voxelSize * voxelSizeScale, voxelSize * voxelSizeScale,
                                           voxelSize * voxelSizeScale);
          downSizeFilterSurf.setLeafSize(2.0 * voxelSize * voxelSizeScale, 2.0 * voxelSize * voxelSizeScale, 
                                         2.0 * voxelSize * voxelSizeScale);
          downSizeFilterMap.setLeafSize(3.0 * voxelSize * voxelSizeScale, 3.0 * voxelSize * voxelSizeScale,
                                        3.0 * voxelSize * voxelSizeScale);
        }
        ...
        if (mapFrameCount >= mapFrameNum) {
          ...
          laserCloudSurround3.header.frame_id = "sensor_init_rot";
        ...
        laserCloudFullRes3.header.frame_id = "sensor_init_rot";
      ...
      if (minStableFrameCount >= minStableFrameNum) odomAftMapped.pose.pose.position.y = transformAftMapped[4];
      else odomAftMapped.pose.pose.position.y = transformRecY;
