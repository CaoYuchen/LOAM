...
double timeOdomProcessedLast = 0;
double timeOdomAftMappedLast = 0;
...
void diffTranslation(float cx, float cy, float cz, float lx, float ly, float lz, 
...
{
  double timeOdomAftMappedCur = odomAftMapped->header.stamp.toSec();
  float timeOdomLasted = timeOdomAftMappedCur - timeOdomAftMappedLast;
  timeOdomAftMappedLast = timeOdomAftMappedCur;

  double roll, pitch, yaw;
  geometry_msgs::Quaternion geoQuat = odomAftMapped->pose.pose.orientation;
  tf::Matrix3x3(tf::Quaternion(geoQuat.z, -geoQuat.x, -geoQuat.y, geoQuat.w)).getRPY(roll, pitch, yaw);

  float shiftX = odomAftMapped->pose.pose.position.x;
  float shiftY = odomAftMapped->pose.pose.position.y;
  float shiftZ = odomAftMapped->pose.pose.position.z;

  if (timeOdomLasted < 5.0) {
    float rx, ry, rz, tx, ty, tz;
    diffTranslation(shiftX, shiftY, shiftZ, transformAftMapped[3], transformAftMapped[4], transformAftMapped[5], 
                    tx, ty, tz, transformAftMapped[0], transformAftMapped[1], transformAftMapped[2]);
    diffRotation(-pitch, -yaw, roll, transformAftMapped[0], transformAftMapped[1], transformAftMapped[2], rx, ry, rx);

    if (!imuInited) {
      transformRec[0] = 0.8 * transformRec[0] - 0.2 * rx / timeOdomLasted;
      transformRec[1] = 0.8 * transformRec[1] - 0.2 * ry / timeOdomLasted;
      transformRec[2] = 0.8 * transformRec[2] - 0.2 * rz / timeOdomLasted;
    }

    transformRec[3] = 0.8 * transformRec[3] - 0.2 * tx / timeOdomLasted;
    transformRec[4] = 0.8 * transformRec[4] - 0.2 * ty / timeOdomLasted;
    transformRec[5] = 0.8 * transformRec[5] - 0.2 * tz / timeOdomLasted;
  }

  transformAftMapped[0] = -pitch;
  transformAftMapped[1] = -yaw;
  transformAftMapped[2] = roll;
  transformAftMapped[3] = shiftX;
  transformAftMapped[4] = shiftY;
  transformAftMapped[5] = shiftZ;
}
...
int main(int argc, char** argv)
{
  ...
  laserOdometry.header.frame_id = "sensor_init_rot";
  laserOdometry.child_frame_id = "laser_odom";

  tf::TransformBroadcaster tfBroadcaster;
  tf::StampedTransform laserOdometryTrans;
  laserOdometryTrans.frame_id_ = "sensor_init_rot";
  ...
  while (status) {
    ...
    if (newCornerPointsSharp && newCornerPointsLessSharp && newSurfPointsFlat && 
      ...
      if (!systemInited) {
        ...
        laserCloudCornerLast2.header.frame_id = "sensor_rot";
        pubLaserCloudCornerLast.publish(laserCloudCornerLast2);

        sensor_msgs::PointCloud2 laserCloudSurfLast2;
        pcl::toROSMsg(*laserCloudSurfLast, laserCloudSurfLast2);
        laserCloudSurfLast2.header.stamp = ros::Time().fromSec(timeSurfPointsLessFlat);
        laserCloudSurfLast2.header.frame_id = "sensor_rot";
      ...
      float timeOdomLasted = timeSurfPointsLessFlat - timeOdomProcessedLast;
      timeOdomProcessedLast = timeSurfPointsLessFlat;

      if (timeOdomLasted < 5.0) {
        transform[0] = transformRec[0] * timeOdomLasted;
        transform[1] = transformRec[1] * timeOdomLasted;
        transform[2] = transformRec[2] * timeOdomLasted;
        transform[3] = transformRec[3] * timeOdomLasted;
        transform[4] = transformRec[4] * timeOdomLasted;
        transform[5] = transformRec[5] * timeOdomLasted;
      }
      ...
      if (laserCloudCornerLastNum > 10 && laserCloudSurfLastNum > 100) {
        ...
        for (int iterCount = 0; iterCount < 25; iterCount++) {
          ...
          float deltaR = sqrt(matX.at<float>(1, 0) * 180 / PI * matX.at<float>(1, 0) * 180 / PI + 0.1 * 0.1 * 2.0 / 3.0);
      ...
      if (timeOdomLasted < 5.0) {
        transformRec[0] = transform[0] / timeOdomLasted;
        transformRec[1] = transform[1] / timeOdomLasted;
        transformRec[2] = transform[2] / timeOdomLasted;
        transformRec[3] = (transform[3] - imuShiftFromStartX) / timeOdomLasted;
        transformRec[4] = (transform[4] - imuShiftFromStartY) / timeOdomLasted;
        transformRec[5] = (transform[5] - imuShiftFromStartZ) / timeOdomLasted;
      }
      ...
      rx = 0.98 * rx + 0.02 * imuPitchLast;
      rz = 0.98 * rz + 0.02 * imuRollLast;
      ...
      if (frameCount >= skipFrameNum + 1) {
        ...
        laserCloudFullRes3.header.frame_id = "sensor_rot";
