...
namespace velodyne_driver
{
   ...
   */
  InputSocket::InputSocket(ros::NodeHandle private_nh, std::uint16_t udp_port):
    Input()
  {
    sockfd_ = -1;
  ...
  int InputSocket::getPacket(velodyne_msgs::VelodynePacket *pkt)
  {
    ...
    pkt->stamp = parseInternalTime(pkt->data[1200], pkt->data[1201], pkt->data[1202], pkt->data[1203]);
        ...
        if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0)
          {
            ...
            pkt->stamp = parseInternalTime(*(pkt_data + 1242), *(pkt_data + 1243), *(pkt_data + 1244), *(pkt_data + 1245));
            empty_ = false;
            return 0;                   // success
          }
  ...
  ros::Time Input::parseInternalTime(uint8_t byte1, uint8_t byte2, uint8_t byte3, uint8_t byte4) {
    char hex1[3];
    char hex2[3];
    char hex3[3];
    char hex4[3];
    sprintf(&hex1[0], "%02x", byte1);
    sprintf(&hex2[0], "%02x", byte2);
    sprintf(&hex3[0], "%02x", byte3);
    sprintf(&hex4[0], "%02x", byte4);
    char hexCorrected[9] = { hex4[0], hex4[1], hex3[0], hex3[1], hex2[0], hex2[1], hex1[0], hex1[1], 0 };
    unsigned long usec = strtol(hexCorrected, NULL, 16);
