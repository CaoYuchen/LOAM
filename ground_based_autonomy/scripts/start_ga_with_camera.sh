...
rosrun realearth_notification notify_user.bash -mw
...
cp $(rospack find clay_cam_v16)/clay_cam_v16.rviz.bak $(rospack find clay_cam_v16)/clay_cam_v16.rviz
cp ~/.data/last_pose.txt ~/.data/last_pose.txt.bak
rm ~/.data/pointcloud.bin ~/.data/pointcloud.num ~/.data/trajectory.ply ~/.data/GPS.ply
roslaunch clay_launch start_mapping_with_camera.launch &
sleep 1
