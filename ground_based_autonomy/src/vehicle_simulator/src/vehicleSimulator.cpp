...
ros::Publisher *pubScanOdometryPointer = NULL;
...
void scanHandler(const sensor_msgs::PointCloud2::ConstPtr& scanIn)
{
  ...
  for (int i = 0; i < scanDataSize; i++) {
    ...
    scanData->points[i].x = pointY3;
    scanData->points[i].y = pointZ3;
    scanData->points[i].z = pointX3;
  }

  geometry_msgs::Quaternion geoQuat = tf::createQuaternionMsgFromRollPitchYaw(vehicleRecRoll, -vehicleRecPitch, -vehicleRecYaw);

  // publish 5Hz odometry messages corresponding to registered scan messages
  nav_msgs::Odometry odomData;
  odomData.header.stamp = ros::Time().fromSec(odomRecTime);
  odomData.header.frame_id = "sensor_init_rot";
  odomData.child_frame_id = "aft_mapped";
  odomData.pose.pose.orientation.x = -geoQuat.y;
  odomData.pose.pose.orientation.y = -geoQuat.z;
  odomData.pose.pose.orientation.z = geoQuat.x;
  odomData.pose.pose.orientation.w = geoQuat.w;
  odomData.pose.pose.position.x = vehicleRecY;
  odomData.pose.pose.position.y = vehicleRecZ;
  odomData.pose.pose.position.z = vehicleRecX;
  pubScanOdometryPointer->publish(odomData);

  tf::StampedTransform odomTrans;
  odomTrans.stamp_ = ros::Time().fromSec(odomRecTime);
  odomTrans.frame_id_ = "sensor_init_rot";
  odomTrans.child_frame_id_ = "aft_mapped";
  odomTrans.setRotation(tf::Quaternion(-geoQuat.y, -geoQuat.z, geoQuat.x, geoQuat.w));
  odomTrans.setOrigin(tf::Vector3(vehicleRecY, vehicleRecZ, vehicleRecX));
  ...
  scanData2.header.frame_id = "sensor_init_rot";
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subTerrainCloud = nh.subscribe<sensor_msgs::PointCloud2>
                                    ("/velodyne_cloud_terrain", 2, terrainCloudHandler);

  ros::Subscriber subSpeed = nh.subscribe<geometry_msgs::TwistStamped>
                            ("/cmd_vel", 5, speedHandler);

  ros::Publisher pubIMU = nh.advertise<sensor_msgs::Imu> ("/imu/data", 5);

  sensor_msgs::Imu imuData;
  imuData.header.frame_id = "imu";
  imuData.linear_acceleration.z = 9.81;

  ros::Publisher pubVehicleOdom = nh.advertise<nav_msgs::Odometry> ("/integrated_to_init", 5);
  ros::Publisher pubScanOdometry = nh.advertise<nav_msgs::Odometry> ("/aft_mapped_to_init", 5);
  pubScanOdometryPointer = &pubScanOdometry;

  nav_msgs::Odometry odomData;
  odomData.header.frame_id = "sensor_init";
  odomData.child_frame_id = "sensor";

  tf::TransformBroadcaster tfBroadcaster;
  tfBroadcasterPointer = &tfBroadcaster;
  tf::StampedTransform odomTrans;
  odomTrans.frame_id_ = "sensor_init";
  ...
  ros::Publisher pubScan = nh.advertise<sensor_msgs::PointCloud2> ("/velodyne_cloud_registered", 2);
  ...
  while (status) {
    ...
    imuData.orientation = geoQuat;
    imuData.angular_velocity.x = 200.0 * (vehicleRoll - vehicleRecRoll);
    imuData.angular_velocity.y = 200.0 * (vehiclePitch - vehicleRecPitch);
