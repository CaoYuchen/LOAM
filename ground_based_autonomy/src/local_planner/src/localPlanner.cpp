...
double odomTime = 0;
double highRateOdomTime = 0;
bool newOdometry = false;
double joyTime = 0;

float odomX = 0, odomY = 0;
float vehicleRoll = 0, vehiclePitch = 0, vehicleYaw = 0;
float vehicleGRoll = 0, vehicleGPitch = 0, vehicleGYaw = 0;
float vehicleX = 0, vehicleY = 0, vehicleZ = 0;
float vehicleGX = 0, vehicleGY = 0, vehicleGZ = 0;
float imuRoll = 0, imuPitch = 0, imuYaw = 0;

pcl::VoxelGrid<pcl::PointXYZI> laserDwzFilter, terrainDwzFilter;

void subHighRateOdomIMU(const nav_msgs::Odometry::ConstPtr& highRateOdom, const sensor_msgs::Imu::ConstPtr& imu)
{
  highRateOdomTime = highRateOdom->header.stamp.toSec();

  double roll, pitch, yaw;
  geometry_msgs::Quaternion geoQuat = highRateOdom->pose.pose.orientation;
  tf::Matrix3x3(tf::Quaternion(geoQuat.x, geoQuat.y, geoQuat.z, geoQuat.w)).getRPY(roll, pitch, yaw);

  vehicleRoll = roll;
  vehiclePitch = pitch;
  vehicleYaw = yaw;
  vehicleX = highRateOdom->pose.pose.position.x - cos(yaw) * sensorOffsetX + sin(yaw) * sensorOffsetY;
  vehicleY = highRateOdom->pose.pose.position.y - sin(yaw) * sensorOffsetX - cos(yaw) * sensorOffsetY;
  vehicleZ = highRateOdom->pose.pose.position.z;

  geoQuat = imu->orientation;
  tf::Matrix3x3(tf::Quaternion(geoQuat.x, geoQuat.y, geoQuat.z, geoQuat.w)).getRPY(roll, pitch, yaw);
...
void highRatePoseHandler(const nav_msgs::Odometry::ConstPtr& highRatePose)
{
  double roll, pitch, yaw;
  geometry_msgs::Quaternion geoQuat = highRatePose->pose.pose.orientation;
  tf::Matrix3x3(tf::Quaternion(geoQuat.x, geoQuat.y, geoQuat.z, geoQuat.w)).getRPY(roll, pitch, yaw);

  vehicleGRoll = roll;
  vehicleGPitch = pitch;
  vehicleGYaw = yaw;
  vehicleGX = highRatePose->pose.pose.position.x - cos(yaw) * sensorOffsetX + sin(yaw) * sensorOffsetY;
  vehicleGY = highRatePose->pose.pose.position.y - sin(yaw) * sensorOffsetX - cos(yaw) * sensorOffsetY;
  vehicleGZ = highRatePose->pose.pose.position.z;
}

void odometryHandler(const nav_msgs::Odometry::ConstPtr& odometry)
{
  double curTime = odometry->header.stamp.toSec();

  double roll, pitch, yaw;
  geometry_msgs::Quaternion geoQuat = odometry->pose.pose.orientation;
  tf::Matrix3x3(tf::Quaternion(geoQuat.z, -geoQuat.x, -geoQuat.y, geoQuat.w)).getRPY(roll, pitch, yaw);

  float curX = odometry->pose.pose.position.z - cos(-yaw) * sensorOffsetX + sin(-yaw) * sensorOffsetY;
  float curY = odometry->pose.pose.position.x - sin(-yaw) * sensorOffsetX - cos(-yaw) * sensorOffsetY;

  float disX = curX - odomX;
  float disY = curY - odomY;

  if (sqrt(disX * disX + disY * disY) > laserCloudSkipDis || fabs(curTime - odomTime) > laserCloudMaxSkipTime) {
    odomTime = curTime;
    odomX = curX;
    odomY = curY;
...
{
  if (acceptLaserCloud && !useTerrainAnalysis) {
    laserCloud->clear();
    ...
    for (int i = 0; i < laserCloudSize; i++) {
      ...
      float dis = sqrt((pointX - odomX) * (pointX - odomX) + (pointY - odomY) * (pointY - odomY));
...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("levelByIMU", levelByIMU);
  ...
  message_filters::Subscriber<nav_msgs::Odometry> claySub(nh, "/integrated_to_init", 1);
  ...
  message_filters::Synchronizer<syncPolicy> sync(syncPolicy(10), claySub, imuSub);
  sync.registerCallback(boost::bind(&subHighRateOdomIMU, _1, _2));

  ros::Subscriber subHighRatePose = nh.subscribe<nav_msgs::Odometry>
                                    ("/integrated_to_map", 5, highRatePoseHandler);
  ...
  ros::Subscriber subTerrainCloud = nh.subscribe<sensor_msgs::PointCloud2>
                                    ("/velodyne_cloud_terrain", 5, terrainCloudHandler);
  ...
  //ros::Publisher pubLaserCloud = nh.advertise<sensor_msgs::PointCloud2> ("/velodyne_cloud_stacked", 2);
  ...
  while (status) {
    ...
    if (newOdometry) {
      ...
      float sinImuRoll = sin(imuRoll);
      float cosImuRoll = cos(imuRoll);
      float sinImuPitch = sin(imuPitch);
      float cosImuPitch = cos(imuPitch);
      ...
      for (int i = 0; i < plannerCloudSize; i++) {
        ...
        if (levelByIMU) {
          ...
          float pointX5 = pointX4;
          float pointY5 = pointY4 * cosImuRoll - pointZ4 * sinImuRoll;
          float pointZ5 = pointY4 * sinImuRoll + pointZ4 * cosImuRoll;

          float pointX6 = pointX5 * cosImuPitch + pointZ5 * sinImuPitch;
          float pointY6 = pointY5;
          float pointZ6 = -pointX5 * sinImuPitch + pointZ5 * cosImuPitch;

          pointX1 = pointX6 * cosVehicleYaw - pointY6 * sinVehicleYaw;
          pointY1 = pointX6 * sinVehicleYaw + pointY6 * cosVehicleYaw;
          pointZ1 = pointZ6;
        }
      ...
      float sinVehicleGYaw = sin(vehicleGYaw);
      float cosVehicleGYaw = cos(vehicleGYaw);
      int boundaryCloudSize = boundaryCloud->points.size();
      for (int i = 0; i < boundaryCloudSize; i++) {
        point.x = ((boundaryCloud->points[i].x - vehicleGX) * cosVehicleGYaw 
                + (boundaryCloud->points[i].y - vehicleGY) * sinVehicleGYaw) / pathScale;
        point.y = (-(boundaryCloud->points[i].x - vehicleGX) * sinVehicleGYaw 
                + (boundaryCloud->points[i].y - vehicleGY) * cosVehicleGYaw) / pathScale;
      ...
      int addedObstaclesSize = addedObstacles->points.size();
      for (int i = 0; i < addedObstaclesSize; i++) {
        point.x = ((addedObstacles->points[i].x - vehicleGX) * cosVehicleGYaw 
                + (addedObstacles->points[i].y - vehicleGY) * sinVehicleGYaw) / pathScale;
        point.y = (-(addedObstacles->points[i].x - vehicleGX) * sinVehicleGYaw 
                + (addedObstacles->points[i].y - vehicleGY) * cosVehicleGYaw) / pathScale;
      ...
      if (autonomyMode) {
        float relativeGoalX = ((goalX - vehicleGX) * cosVehicleGYaw + (goalY - vehicleGY) * sinVehicleGYaw);
        float relativeGoalY = (-(goalX - vehicleGX) * sinVehicleGYaw + (goalY - vehicleGY) * cosVehicleGYaw);
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        ...
        if (selectedGroupID >= 0) {
          ...
          path.header.stamp = ros::Time().fromSec(highRateOdomTime);
          ...
          freePaths2.header.stamp = ros::Time().fromSec(highRateOdomTime);
      ...
      plannerCloud2.header.stamp = ros::Time().fromSec(highRateOdomTime);
